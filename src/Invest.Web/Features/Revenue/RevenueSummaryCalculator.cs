...
public static class RevenueSummaryCalculator
{
    ...
    ///
    /// 2026-09-02 曾經把這條改成「逐檔取自己手上最新的一期」，想解決「9/01 整欄變空白」。
    /// 那是**修錯地方**：整欄空白的真正原因是抓取漏了早公告的那幾十家（見
    /// <c>RevenueClient</c> 的「公告期要走觀測站」一節），不是這條規則太嚴。
    /// 規則放寬只會讓畫面混著兩個月份，把資料沒抓到這件事蓋掉，所以已經改回來。
    /// </summary>
