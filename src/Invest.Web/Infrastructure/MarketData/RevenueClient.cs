...
///     一次就把全部公司的最新已公告月份給完，含 KY 股。整月的主力走這條。
///   指定月份（<see cref="GetMonthAsync"/>）——公開資訊觀測站的逐月報表，
///     一個月要抓四個檔案（上市／上櫃 × 國內／外國企業）。回補歷史走這條。
///
/// 公告期要走觀測站
/// ----------------
/// 上面那兩支 OpenAPI 是**整批出表**的，出表日期就寫在回應裡。2026-09-02 當天問到的
/// 1085 列全部是「出表日期 1150817、資料年月 11507」，也就是 8/17 那份七月的完整名單。
/// 換句話說 1~10 日先報的那幾十家，OpenAPI 要等到下一次出表（約 17 號）才收得進去。
///
/// 觀測站的逐月報表則是即時的：同一天問 115 年 8 月，sii 26 KB、otc 22 KB，
/// 用同一組 <see cref="MonthlyRowPattern"/> 解得出 49 檔已公告。
/// 只靠 OpenAPI 的話這 49 檔在網站上就是「明明已經公布了卻沒抓到」（筆記 #47）。
///
/// 所以定期更新是兩條一起走：OpenAPI 拿全量，再用觀測站補上個月那一格。
/// 這件事寫在 <c>Program.cs</c> 的 <c>RunRevenueAsync</c>。
///
