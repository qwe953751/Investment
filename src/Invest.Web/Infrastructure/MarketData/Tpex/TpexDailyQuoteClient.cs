...
public sealed class TpexDailyQuoteClient(HttpClient httpClient, ILogger<TpexDailyQuoteClient> logger)
{
    ...
    /// <summary>
    /// 欄位順序：0 代號、1 名稱、2 收盤、3 漲跌、4 開盤、5 最高、6 最低、
    ...
    private static DailyQuote? ParseRow(JsonElement row)
    {
        ...
        return new DailyQuote
        {
            ...
            OpenPrice = QuoteFieldParser.ParseNullableDecimal(row[4].GetString()),
            HighPrice = QuoteFieldParser.ParseNullableDecimal(row[5].GetString()),
            LowPrice = QuoteFieldParser.ParseNullableDecimal(row[6].GetString()),
