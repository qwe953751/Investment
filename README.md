...
最高權限頁籤現在提供「資產 Dashboard → 帳戶明細」兩層的**瀏覽器樣板**；使用者、帳戶與已確認金額
只留在目前瀏覽器，截圖不會上傳或保存，OCR 也尚未接入。檢視權限不顯示此頁籤，這不代表持倉管理
已納入正式資料範圍。`review-20260826-assets-v1` 只保留作為本機筆記假資料與版面驗證開關。
...
「資產」是最高權限樣板中的瀏覽器資料工作區；可在 Dashboard 與帳戶明細間切換、新增使用者／帳戶，
並用截圖作為人工校對未實現損益的輸入。資料只存在目前瀏覽器的 `localStorage`，不會寫入 Supabase、GitHub Pages
或券商；正式帳戶同步、OCR、登入與 RLS 仍未實作。資產頁保留上方主頁籤，方便切回其他功能頁。
...
筆記圖片功能目前在本機實作中，需先獨立套用 `db/022_notes_images.sql` 才會啟用正式 Storage 上傳；`note-images` 沿用目前筆記的公開 anon 模型，請勿上傳含帳號、密碼或敏感金融資訊的原始截圖。
