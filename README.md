...
不提供編輯或刪除；歷史年度可在帳戶明細新增年份、填寫總資產／入金成本、編輯總資產或刪除整列。
Dashboard 的年度區塊只讀，總資產與入金成本由各帳戶年度資料逐年彙總，排序固定由新到舊；
新增只接受早於當年度的年份，避免把當年度自動值寫成可手動覆蓋的歷史資料。
年度表的 `cost_twd` 欄位代表入金成本（入金合計 − 出金合計），不是持倉投入成本；正式前端只使用
`account_id` scope 寫入與彙總，舊有 `owner_id` 欄位僅保留資料表相容性，Dashboard 不採用 owner 年度列。
