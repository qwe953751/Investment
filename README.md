...
本次「自訂資料瀏覽」的合併提交是 `a7698a0`（2026-08-21），正式網站快照版本是 `1787281973`。
這一版已完成並驗收：

- 盤中、盤後旁新增「自訂」頁；可瀏覽單一交易日的完整上市櫃資料，不建立預設排行。
- 自訂頁顯示收盤價、營收增長（YOY／MOM）與成交值，可選日期、設定成交值下限，並以每頁 100 檔分頁。
- 盤中／盤後加權指數、櫃買指數及漲跌幅。
...
- 漲跌幅（日／週）與營收增長（YOY／MOM）共用同一套上下層排版：上層 16px、下層 14px；排序仍分別使用日漲跌幅與 YOY。
- 表格不再設獨立市場欄；股票代號右側以「市／櫃」小標記區分上市與上櫃，市場篩選功能維持不變。
- 盤中 MIS 當日 K 棒、指數與同一個 2 分鐘更新計時器同步刷新。
- `db/007_market_indices.sql`、`db/008_intraday_kline.sql` 已套用 Supabase。
...
與盤後排行的平均每日成交門檻分開保存；營收增長沿用排行頁的 YOY／MOM 格式，
摘要會標示目前應公告的營收月份。
