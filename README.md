...
| 預估收盤 | 今日累計成交額 ÷ 時段進度 |
| 倍數 | 預估收盤 ÷ 過去 N 日的平均每日成交值 |

比例比得起來、金額比不起來：半天的量拿去比人家一整天的量一定小。
先把今天推估成全日再比也不行，那會把準確度整個綁在還沒校正的線性預估上。
市場成交比的分子與分母取自**同一輪**，時段進度會自己約掉，所以 09:05 就能看。

因此**排行一律以實際累計成交額或成交比變化為準**，預估收盤與倍數只是參考欄，**不開放排序**：
它們的分子帶有預估值的誤差，排序等於變相鼓勵拿失真的數字排名次。
09:27（時段進度 10%）以前分母太小，預估欄一律顯示 `—`。

## 處置股標記

名稱前面的紅色「處」代表這一檔**目前**處於處置期間。處置會把撮合改成人工分盤
（現在是每 2 分鐘一次，115/08/10 之前第一次處置是每 5 分鐘），成交機會被人為壓低，
所以它在成交值排行上的名次不能照字面讀。滑鼠停上去會顯示處置期間與撮合頻率。

**注意股不標**：注意股不改變撮合方式，成交值照樣是自由競價的結果。

資料在 export 時抓，寫進 `manifest.json`，盤中與盤後共用同一份：

| 市場 | 來源 |
|---|---|
| 上市 | `twse.com.tw/rwd/zh/announcement/punish` |
| 上櫃 | `tpex.org.tw/openapi/v1/tpex_disposal_information` |

兩邊都只回「現在誰在處置中」，沒有歷史查詢，所以往回選基準日時這個標記講的仍然是現在的狀態。
