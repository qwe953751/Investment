...
- `db/014_market_heat_turnover_change.sql` 已套用；它保存盤中預估收盤成交額及其相較前一交易日正式成交額的變化，盤後不顯示這個比較。
...
- 族群熱度排行點擊名稱會在目前排行列下方展開／收合個股明細，不自動跳到其他頁籤，也不建立第二層彈窗；明細支援營收增減與漲跌幅排序。
- 發布流程已將 `publish-only=true` 與完整快照分開併發鎖；完整快照正在回補／備份時，純畫面發布不必取消資料流程或等待它結束。

## 最近已整合功能（已發布）

目前本機正在驗證族群頁的改動：熱度排行新增「名次變化」，點擊族群名稱就在目前排行表內展開／收合完整個股列表，不自動切換到「族群列表」頁籤，也不建立第二層族群彈窗；族群列表的個股表也新增「營收增減（YOY／MOM）」與「漲跌幅」排序。盤後名次變化比較前一個相同長度的觀察區間；盤中沒有可比較前輪時顯示 `—`。
