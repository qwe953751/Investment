...
名稱**底下**掛的標記講的是這一檔**目前**受到的交易限制。放在名字底下而不是左邊，
是因為放左邊會把有標記的那幾列往右推，整欄的名字就對不齊了。

| 標記 | 意思 | 為什麼會壓低成交值 |
|---|---|---|
| 處置（紅） | 處置期間 | 撮合被改成人工分盤（現在每 2 分鐘一次，115/08/10 之前第一次處置是每 5 分鐘） |
| 全額（橘） | 變更交易方法 | 買賣都要先付足款券、不能融資融券，願意接手的人本來就少 |
