...
四市場已共用 C# 熱絡計算引擎與市場設定。日股／韓股的兩年盤後日線回補與 5 分鐘盤中快照皆已接線：盤後資料只寫 `data` 分支快取，盤中只寫獨立 Supabase Storage CDN（不寫 Postgres），前端只在當地交易時段且快照未過期時優先顯示盤中暫估，否則回退到共同日期的盤後日線。日本使用 TOPIX-17 ETF 代理產業，韓國明確標示為 11 檔產業代表；兩者不是市值權重。Yahoo Finance 是無 SLA、可能限流的候選來源；本次在週日完成程式、測試與推送，但未觸發網站發布，也尚待下一個交易日首次實際回補與 Storage 驗收。
