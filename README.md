...
持倉管理仍由最高權限資產頁處理；另外提供「持倉檢視者」唯讀視角，只呈現 Frank
所有帳號的持股與市場篩選，不延伸成投資建議或買賣訊號。網站的族群頁只把既有個股行情
重新聚合成市場資金流。
...
- 「自訂」是台股資料的主頁籤，頁內可切換個股／ETF；資料時間選項固定先顯示盤中、再顯示盤後。個股沿用正式網站的收盤價、漲跌幅、營收增減、創高月數與成交值欄位；ETF 可選交易日或最新盤中快照，收盤價／現價、K 線、日／週／年漲跌幅與成交值會跟著時段切換，且不混入個股成交值排行統計。
...
GitHub 組織，不含原始帳號名稱）。打開就是**訪客（檢視）模式**，持倉檢視者／監控者／最高權限
一律要在頁首右上角登入才能拿到（見下方「密碼登入權限」）；密碼登入是唯一的
存取控制，網址本身不再區分權限。

長者友善連結可使用 `https://frank-invest.github.io/?key=<密碼>` 免輸入密碼登入。網址帶有明確的
`key` 時，啟動會先驗證這次連結的密碼，成功後直接切換到該帳號權限，即使同一裝置原本留有其他
登入的 refresh token 也一樣；只有 `key` 驗證失敗才回復原本的 session，沒有 `key` 才單純自動恢復
既有 session。驗證後 `key` 會從網址列移除，不會持續留在分享連結或瀏覽紀錄中。
...
不是登入授權。
「持倉檢視者」登入後只顯示筆記 #53 附件模板：資料固定取 `資產 → Frank → 所有帳號持股`，
市場頁籤可切換台股／美股／加密貨幣；八欄的欄位內容與互動共用「盤中」成值排行榜：名稱可開同一個
K 線、族群可跳到同一個節點、營收可開同一個 20 個月彈窗，代號沿用市場／交易限制標記，漲跌幅沿用
...
使用者可一次選最多 20 張；Tesseract 每張最長辨識 10 秒。辨識中可按「強制取消辨識」立即停止本次批次，
不會套用任何持倉變更。完成後會列出和既有持倉的覆蓋／新增／移除差異，覆蓋與新增預設自動勾選，
人工修正後按「確認修改並更新差異」重新確認；移除預設不勾選。同代號直接覆蓋，只有確認的持倉數字寫入 Supabase，
原始截圖不會寫入持倉資料，也不會送往券商；AI 路徑會由本機訂閱 CLI 送至其模型服務。
最高權限資產頁保留上方主頁籤，方便切回其他功能頁；持倉檢視者則固定在唯讀持倉頁。
匿名 RLS 收權限仍待驗收，故本次新增的是前端登入視角與唯讀呈現，不宣稱資料表已完成隔離。
Dashboard 的資產圓餅圖中心會依格式化後金額的字元長度縮放總資產與未實現損益，內圈保留邊界，不會讓數字碰到圓環。
...
更新後都會重新依代號排序。
輸入已知台美股或 ETF 代號時會帶入名稱；名稱底色、右側漲跌幅、現價／市值與持倉 K 線共用同一套
行情交接：交易日開盤後使用當日盤中行情，收盤後到官方盤後版本發布前（通常 18:00）仍沿用當天最後
一輪盤中資料，版本發布後才切換最近收盤。未實現損益固定為「市值 − 成本」，顯示為金額(%數)時隱藏
正負號，方向由既有紅漲綠跌色塊表示。休市日不會把上一個交易日留在 `intraday_latest` 的資料誤標為盤中。
...
Dashboard 與帳戶明細的「每年總資產與試算淨值」共用同一個年度元件，資料表為
`db/048_asset_annual_snapshots.sql` 的 `asset_annual_snapshots`：區塊預設收合，點擊年化報酬卡才展開；
當年度每次依目前資產狀態自動帶入，不提供編輯或刪除。歷史年度可在帳戶明細新增年份、填寫總資產、
編輯總資產或刪除整列；入金成本不手動輸入，而是依該曆年的出入金紀錄（入金合計 − 出金合計）自動帶入。
...
Dashboard 不採用 owner 年度列。Dashboard 帳戶表的「總獲利」＝資產總值－入金成本；帳戶內的實際已實現損益仍是另一個欄位。
最高權限帳號初次進入資產頁時，會依登入身分預設選取對應的使用者（Frank 或財神），仍可手動切換；
Dashboard 與帳戶明細的折線圖共用版本 A：滑鼠移動／點擊或鍵盤聚焦日期點時，標題區會顯示選定交易日期、總資產、
與前一斷點的差異金額與百分比；十字線預設開啟且可切換，底部保留期間按鈕與日期範圍。
年度區塊展開後，每一張年度卡可個別查看「每月 MOM」柱狀圖；圖表只使用對應 Dashboard 使用者或帳戶的資產日快照，
每月取該月最後一筆快照計算月變化，當年度補上目前資產總值作為最新月份資料。月份固定由左到右升冪排列：當年度從 1 月到最新月份，
歷史年度為 1 月到 12 月；沒有前一月份快照時顯示「—」，不以年度總額或成本推估。圖表不顯示額外的年度前綴、色彩圖例或資料說明列。

本機預覽：`http://127.0.0.1:5220/?access=admin`、
`http://127.0.0.1:5220/?access=viewer`、
`http://127.0.0.1:5220/?access=holdings`（持倉檢視者模板）。
...
- Podcast／股癌正式採用方案 E，放在筆記子頁籤：進入 `?view=notes&notesTab=gooaye` 可查看跨集宏觀總覽、研究概念／市場情境／待驗證三欄與集數演進。集數演進使用左側日期／集數節點、垂直連線與逐集白色內容卡；來源頁只要求輸入時間、集數與 Gemini Notebook 逐字稿分析。來源透過 Supabase 儲存，支援新增、編輯、移除，多筆匯入與單筆編輯面板都在集數演進區域就地開啟並自行捲動，其餘欄位由內容解析產生預覽，尚未串接正式 Gemini API。
- 股癌頁籤的手機版小控件會固定在市場導覽的最上層，切換市場時不會被後續內容誤接收點擊；管理員裝置清單會以「裝置名稱＋IP＋權限」保留最新列並清理重複舊列；其他市場的彈出式 K 線標題也一律連到既有 MoneyDJ 財經百科查詢。
...
筆記 #64／#65 已完成：U1 手機版的「裝置」浮層改以全寬工具列左側定位，避免面板在手機上從左側被裁掉；盤中摘要的前五個歷史熱絡分數改為可操作按鈕，點擊後會切到「盤後」並載入該按鈕對應的交易日。既有熱絡資料查詢、盤後日期載入與 Supabase schema 均不變；按鈕也保留鍵盤 focus、提示與無障礙標籤。本輪程式、Node 76/76 與 .NET 10.0.302 457/457 驗證完成，正式發布資訊見下方最新版本與 [版本紀錄](Doc/版本紀錄.md)。

本輪 K 線版面與查價線已完成實作：成交量區底部空白再收斂，圖表只保留日期標籤所需的下緣空間；
現價旁的漲跌幅依漲紅、跌綠、平盤黑顯示。個股 K 線新增「成本」查價線，預設開啟，只有最高權限
可見與切換；成本由登入最高權限對應的資產使用者（Frank／財神）在目前市場的所有子帳戶持股，
以總成本除以總股數計算，並以青綠虛線與價格線區分。非最高權限不會取得或繪製成本資料。
本輪程式與文件已完成本機驗證，`main` commit `f0a9bdc6360d4c3313ebb80b01de72705bc5239d` 已推送，並由
[`daily-snapshot.yml` publish-only run 34440217022](https://github.com/qwe953751/Investment/actions/runs/34440217022)
完成正式網站更新；Pages 與公開 manifest 均為 `1789017300`，公開 `site.js` 已核對成本查價線與緊湊圖表接線。
兩組最高權限 Supabase Auth 登入組合已驗證可用，密碼只保留在 Auth，不寫入前端、文件或版本庫。
...
四市場已共用 C# 熱絡計算引擎與市場設定。日股／韓股的兩年盤後日線回補與 5 分鐘盤中快照皆已接線：盤後資料只寫 `data` 分支快取，盤中只寫獨立 Supabase Storage CDN（不寫 Postgres），前端只在當地交易時段且快照未過期時優先顯示盤中暫估，否則回退到共同日期的盤後日線。日本使用 TOPIX-17 ETF 代理產業，韓國明確標示為 11 檔產業代表；TOPIX／JPX-Nikkei 400／KRX 100 與韓國風險值的代理語意均寫在市場名稱與 warning，20% 風險權重不因缺來源而消失。下載器會在核心資料不全時失敗且不提交部分快取；日韓日線已完成兩年回補並發布至正式網站。市場總覽 JSON 的正式 wire key 固定為 `jp`／`kr`（前端對舊 `japan`／`korea` 快照只保留一次相容讀取）；盤中 Storage 首輪已寫入 `jp`／`kr` 快照並由最新 manifest `1789354845` 指向，後續依交易日持續更新。
美股／日股／韓股的「成交金額前 20」已接入獨立來源與快取契約：美股使用 Massive grouped daily aggregates，日股／韓股使用 KIS 的 TSE／KRX 成交金額排行；集中 collector 只在至少 20 列、名次連續、代號不重複且金額有效時寫入 `data/imports-turnover`。盤後排行在 export 時併入 `market-overview.json`，盤中排行只送版本化 Supabase Storage CDN，瀏覽器不直連來源、不查 PostgreSQL。缺少 Actions secrets、來源 429／404 或資料不足時 workflow 失敗，不會用指數／產業代表或本機示意列補值；公開 CDN 預設關閉，確認授權後才開啟。本輪完成程式與測試，但依指示未回補真實資料、未 export、未發布網站。
...
`2026/09/10`、產生時間 `2026-09-11 09:16`；線上 `site.js` 已核對包含前五日熱絡日期按鈕、盤後交易日導覽與 U1 手機裝置浮層左側定位。publish-only 的行情回補、Supabase 同步、備份與心跳均依設計略過。
...
- 手動觸發 `daily-snapshot.yml` 的預設是 `publish-only=true`（只重新輸出並發布，不等今日行情）；要連同今天的行情一起更新才取消勾選，且須等收盤行情公布。每天三場排程是 `schedule` 事件、不套用這個預設值，照常跑完整流程。15:00 前以 `publish-only=false` 啟動會被守衛直接擋下，不會進回補迴圈空轉到晚上。
- 資產截圖可一次選最多 20 張，逐張最長辨識 10 秒；OCR 結果先列出覆蓋／新增／移除差異，逐項核對並勾選後才可套用。同代號直接覆蓋，移除預設不勾選，避免 OCR 漏列誤刪持倉。
- 台股 ETF 以證交所／櫃買中心官方名冊判定：保留在資產名冊、ETF 逐日快照與日 K，排除一般股票成交值排行及市場成交值分母；ETF 盤中行情另以 `kind` 分流；可用 `backfill-etfs` 補齊既有快取。
...
寫入資料庫的完整原始輪次輸出為不可覆寫的 `intraday-YYYYMMDD-HHmm-runN.json`，再更新極小的
`latest.json` 指標；瀏覽器只輪詢後者，run id 改變才從 CDN 下載完整快照。原始快照包含全市場
所有個股、指數與市場熱絡，前端再做篩選與排序，因此不是「固定前一百檔」而是完整、可重現的排行。

族群熱度是原始報價的非同步衍生流，不阻塞下一輪 MIS 收集。背景處理器依 raw run id 從
Supabase 找出尚未完成的輪次，分類／計算成功後另存不可變的
`intraday-topic-YYYYMMDD-HHmm-runN.json`，並更新 `topic-latest.json`。因此 raw 的
`latest.json` 可以先前進，族群頁則保留上一份完整族群快照並顯示「計算中」；topic run 追上後，
前端會自動換成同一輪資料，不會以 `topicHeat: null` 清空或覆蓋完整快照。

GitHub Actions 在正式收集前會執行 `intraday --probe`，沿用同一份全市場清單與 150 檔批次，
不再用單一 2330 探測代表整個 MIS。正式收集器只接受至少 80% 的報價覆蓋；若連續三輪失敗會
提前退出並留下網站警報，讓自走鏈的下一個 runner 接手，不再把 502 或殘缺回應拖到收盤才發現。

原始與族群完整快照都採不可變檔名，各自只保留最近 30 份（約 60 分鐘）；`latest.json`、
`topic-latest.json` 與未知檔案不會被清理。這避免每兩分鐘一份 JSON 無限制累積並耗盡 Storage
額度，同時保留足夠時間讓已讀到上一個指標的瀏覽器安全完成下載。

這條 CDN 路徑只在選擇盤中資料時啟用：**盤中排行、自訂資料瀏覽→盤中、族群→熱度排行→盤中、
族群→族群列表→盤中**（以及列表中展開的盤中個股 K 線）。筆記、資產、提醒、營收、盤後排行、
非盤中的族群頁面都維持原本的資料來源與更新方式。使用者端在交易時段、頁面在前景時輪詢；若
raw run 已前進而 topic 尚未追上，收盤後仍會繼續只讀輪詢直到族群快照完成，否則收盤後停止。
同瀏覽器分頁以 `BroadcastChannel` 共用 raw 與 topic 更新，舊分頁每十分鐘檢查網站版本。

多個喚醒事件可能同時啟動非同步抓取；前端套用快照時會以遞增 `runId` 及 `captured_at`
拒絕較舊的回應，資料庫 fallback 也不會把已知較新的快照覆蓋掉。這確保「資料時間」與下方列表
始終來自同一輪，不會因請求完成順序不同而出現時間前進、列表倒退的畫面。
...
盤中、盤後、自訂與族群檢視共用同一份瀏覽器端營收狀態；成功讀取後最多每 15 分鐘更新一次，
更新完成會立即重畫目前檢視。第一次讀取失敗會每分鐘重試；已有成功資料時保留上一份結果，
不把暫時性的連線錯誤誤顯示成沒有營收資料。
