...
市場頁籤可切換台股／美股／加密貨幣；表格不提供使用者選擇、資料說明、名次變化、帳戶明細、
刪除、編輯或清除操作，右上工具列沿用訪客可見的控制項。
...
不再接受人工輸入容易過期的名稱、市值與損益。預設按代號排序，可切換按日漲跌排序，新增或 OCR
更新後都會重新依代號排序。
輸入已知台美股或 ETF 代號時會帶入名稱；名稱底色與右側日漲跌在交易日盤中使用當日盤中行情，盤後則使用
最近收盤。日漲跌是行情相對前一有效收盤的變化，不是持倉報酬率；未實現損益則以市值與成本計算，
並保留金額／報酬率的正負號。休市日不會把上一個交易日留在 `intraday_latest` 的資料誤標為盤中。
