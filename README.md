...
- Podcast／股癌正式採用方案 E，放在筆記子頁籤：進入 `?view=notes&notesTab=gooaye` 可查看跨集宏觀總覽、研究概念／市場情境／待驗證三欄與集數演進。來源頁只要求輸入時間、集數與 Gemini Notebook 逐字稿分析；來源透過 Supabase 儲存，支援新增、編輯、移除，多筆匯入與單筆編輯面板都在集數演進區域就地開啟並自行捲動，其餘欄位由內容解析產生預覽，尚未串接正式 Gemini API。
