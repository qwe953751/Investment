...
| 5 | [盤後「抓到有為止」碰上國定假日會誤紅燈](#todo-5) | 🟡 休市日閘門已完成，待下一交易日驗收 |
...
**狀態：休市日閘門與品質隔離已完成，待下一個實際開盤日驗收。**
2026-09-22 已把日韓盤中、成交排行與歷史匯出共用交易所休市日曆；日本 9/22～9/23
不再呼叫 Yahoo、寫入 Storage 或計入異常輪次。前端也會在休市日停用盤中請求，並拒絕
排行日期與總覽日期不一致的 cached rows。2026-09-24 開盤後需確認 JP 首輪快照能從
9/18 正確切到 9/24，且不再產生 9/22／9/23 偽交易日檔案。
2026-09-14 另補上「開盤日盤中啟動」這個漏網情境，守衛與預設值已改，
但兩者都還沒在真實觸發下驗收過（見下一節）**

### 2026-09-14 更新：開盤日的盤中手動觸發不再空轉到晚上

觸發原因：2026-09-14（週一，開盤日）台北 11:29 手動觸發 `daily-snapshot.yml`
且沒有帶 `publish-only`，「回補行情」一路重試，卡了 21 分鐘後由人手動取消
（run `34802804075`）。

根因**不是** 2026-09-12 那條的漏修。三層判定這次完全正確：今天確實是開盤日，
回傳 `open`。問題是「今天開不開盤」回答不了「今天的收盤行情公布了沒」——
官方收盤行情約 15:00 才公布，11:29 進到回補迴圈就是每 10 分鐘白試一次、
空轉到 21:00 才紅燈，總共 9.5 小時。2026-09-12 修掉的是休市日空等，
這次是**開盤日但還沒到公布時間**，是同一個迴圈的另一個入口。

真正把人引到這裡的是預設值：`publish-only` 原本 default `false`，而手動觸發
幾乎都是「把目前 `main` 發出去」——AI agent 走 `gh workflow run ... -f publish-only=true`
不受影響，但族群人工編輯頁「立即發布」連結是直接開 GitHub UI 按 Run workflow，
要自己記得打勾。漏勾就是完整流程。

**做法**（三項，同一個 commit）：

1. `publish-only` 的 default 改成 `true`。GitHub UI 沒辦法用 query string 預填
   `workflow_dispatch` 的 inputs，checkbox 初始狀態只能由 default 決定，所以
   這是唯一能做防呆的地方。排程是 `schedule` 事件、**不套用** `workflow_dispatch`
   的 default，每天那三場照樣跑完整流程；`intraday.yml`／`us-daily-snapshot.yml`
   自走鏈與 AGENTS.md 的指令都是顯式帶參數，也不受影響。
2. `site.js` 族群編輯頁的引導文案同步改掉。預設已經勾好時還寫「打勾 publish-only
   再送出」，使用者很容易當成要切換而反手點掉，正好製造出這次的故障——
   所以這一項是強制連動，不是可選。測試 `族群編輯頁的立即發布引導不再要求自己打勾`
   會盯著這兩邊不准脫鉤。
3. 新增「擋下盤中啟動的完整流程」守衛步驟，放在回補迴圈之前：手動觸發、
   `publish-only=false`、今天不是確定休市、而且還不到 15:00 時直接 `exit 1`。
   門檻取 15:00 而不是排程自己等的 18:00，是因為 15:00～18:00 之間手動補今天的
   行情是合理的，不該一併擋掉。這一層只在「顯式把預設值關掉」時才會遇到，
   機率很低，但它是唯一還會空轉的路徑，幾行 bash 換掉 9.5 小時。

**尚未驗收**：`publish-only` 新預設值要等下一次手動觸發才看得到實際行為；
守衛要等真的在 15:00 前帶 `publish-only=false` 才會被執行到。更要緊的是
確認每天 16:07／18:17／18:47 那三場排程仍走完整流程——如果 `schedule` 事件
意外套用了 default，症狀會是「行情不再更新但 Actions 全綠」，很難察覺，
所以下一個交易日要實際核對 `data` 分支有沒有新增當天的 `imports/*.json`。

### 2026-09-12 更新：三層判定收斂成一個入口，週末手動觸發不再空等 5.5 小時

觸發原因：2026-09-12（週六）手動觸發 `daily-snapshot.yml`，「回補行情」
卡在第 4 次重試（「還沒有 2026-09-12 的行情，10 分鐘後再試」），因為
這一步完全不看「今天是不是週末」，會一路傻等到 21:00 才紅燈。

根因是**三套非交易日判定各管各的、互不相通**，而且沒有一套算得出
「政府臨時宣布不上班」（颱風假）：
`daily-snapshot.yml` 的「回補行情」只看今天的行情檔案存不存在，
...
這一層專門抓颱風假這種**排程當下查不到、只有臨時公告**的情況。
任何一層問不到答案就回傳 `Unknown`，呼叫端維持原本的重試行為——
誤判成休市會被寫進快取而不再重試，代價遠比多等一輪大。

新增兩個 CLI 命令：

- `market-day`：印出今天的判定，並用 exit code 回答（0＝開盤、
  10＝確定休市、20＝不確定），讓 GitHub Actions 不必解析文字輸出。
  `daily-snapshot.yml` 在「回補行情」前新增「判斷今天是否開盤」步驟，
  結果是 `closed` 才跳過整個回補迴圈，不再空等到 21:00；`unknown` 則
  完全不影響原本行為。
- `export-market-calendar`：把證交所休市日曆匯出成 `data/market-calendar.json`，
  每天由 `daily-snapshot.yml` 更新、推回 `data` 分支。`intraday.yml` 開盤前
  純 bash 的等待迴圈（`in_session()` / `next_open()`）直接 `curl` 這份快取
  跳過已知的平日假日，不必為了問一句「今天是不是假日」冷啟動 `dotnet run`；
  讀不到就當作沒有已知假日，退回原本純靠時段判斷的行為，不會誤判。
  颱風假這種抓不到的臨時公告，仍然只能靠收集迴圈自己的 MIS 停滯偵測抓，
  這條路徑本身沒有改動。

盤中頁在假日的顯示行為刻意不變：維持顯示上一個交易日的快照，
直到有新的盤中資料取代，不額外顯示「今天休市」。
...
### 2026-09-10 又查到的 MIS runner 故障

當日開盤時舊 runner 對 MIS 的每批 150 檔請求連續回 HTTP 502；單檔 2330 探測仍可能成功，
所以原本的探測不足以代表全市場。舊收集器把每輪例外記下後繼續等下一輪，造成一直列隊、直到收盤才讓 workflow 失敗。

已實作的修正：

- workflow 改用 `intraday --probe`，沿用實際個股清單與 150 檔批次，探測失敗立即讓本棒失敗；
- 正式收集器把低於 80% 報價覆蓋視為殘缺輪次，不寫入快照；
- 連續三輪失敗立即退出，讓 `更新網站上的警報` 執行並由下一個 runner 接手；一兩輪暫時性失敗仍保留原本的兩分鐘重試。

本次選擇快速退出與完整探測，而不是先增加批次切割或無限重試：現場是整條 runner／MIS 路徑一致 502，
增加請求只會延長佔用與放大來源壓力。完整批次探測與下一交易日實跑後，再決定是否需要針對單批做更細的 fallback。
...
筆記 #61（2026-09-11）已把整期用量歸因完畢，見下面「2026-09-11 用量歸因」。
...
### 2026-09-11 用量歸因（筆記 #61）

計費週期 08-15～09-14。期末 Egress 8.86 / 5 GB（超額 3.86 GB）、
Edge Function 443,155 / 500,000、Cached Egress 0.12 GB、Storage 0.01 GB。
Dashboard 的每日圖把成因拆得很清楚，**是兩件事，不是一件**：

| 期間 | 每日 | 小計 | 成因 |
|---|---|---|---|
| 08-15 ～ 08-24 | 0.05～0.4 GB | ~1.5 GB | 盤中全市場直連資料庫（CDN 改造前的基線） |
| **08-25** | **~4.2 GB** | **~4.2 GB** | 開發日多分頁，見下面「8/25 已結案」 |
| 08-26 ～ 09-05 | 趨近 0 | ~0.3 GB | CDN 改造生效 |
| 09-06 ～ 09-11 | 0.3～0.6 GB | ~2.5 GB | D+ OCR Worker 版本落後，見 [TODO 15](#todo-15) |

- **超額的 3.86 GB ≈ 08-25 那一天。** 沒有那一天，這一期是 ~4.6 GB，根本不會超。
- **Edge Function 的 443,155 次 100% 出自 09-05 之後**（08-15～09-04 是零，柱狀圖加總等於整期用量）。
  09-09、09-10 各約 140K 次＝**1.6 次／秒**，來源是版本落後的 Windows OCR Worker：
  EXE 建於 09-09 00:57，比事件驅動修正（`9654ab3f`，09-09 18:10）早 17 小時，
  一直在跑舊的 2 秒輪詢迴圈（空轉一輪＝1 heartbeat ＋ 3 並行槽 × 3 claim ＋ 1 evaluation-claim
  ≈ 11 次／6 秒）。09-11 11:49 已停程序並 `Disable-ScheduledTask` 止血。
- **Worker 會同時吃掉 egress**，因為 `ocr-jobs` 每被呼叫一次就會再往 `/rest/v1/...` 打一次內部請求
  （`handleClaim` 打 `rpc/ocr_claim_job`、`handleHeartbeat` 打 `ocr_workers`）。所以每日圖上
  綠色（Database）與黃色（Functions）是一起長出來的，實測約 **3 KB／次呼叫**。
- 09-08 那根特別高（invocation 反而比 09-09／09-10 少）是那天真的在跑 OCR：
  截圖走 private bucket 的簽名網址，不進 CDN、算 uncached egress，一張 1～3 MB。這是使用者驅動的一次性成本。

**結論：這已經不再是架構問題。** CDN 改造後的結構性基線只有 0.02～0.05 GB／天；
下一期（09-15～10-14）在兩台 Worker 都修好的前提下推估 0.6～1.5 GB／月，
遠低於 5 GB，也低於 9/27 的 5.5 GB 門檻。
...
**先不要打，留著當備案**；等新週期實測真的接近 5 GB 再拿出來。
...
### 8/25 已結案（2026-09-11，筆記 #61）

原本卡在「34,880 次 REST 請求是平常的 6 倍，原因還沒查出來；如果是重試風暴，
換到 CDN 之後會原封不動複製過去」。**不是重試風暴，是開發日的多分頁**，
而且當天三個節流機制一個都還沒進版：

| 機制 | 進版 commit | 08-25 當天 |
|---|---|---|
| 盤中欄位拆分（2.1 MB → 339 KB，−84%） | `39b3c132` 08-26 19:27 | ❌ 還在抓 **2.1 MB／輪** |
| 同瀏覽器多分頁租約（N 個分頁 → 1 個輪詢者） | `e5e1b8db` 08-29 | ❌ 每個分頁各自輪詢 |
| 盤中走 Storage CDN | `6a8e1d3d` 08-30 | ❌ 每輪直連資料庫 |

再加上當天營收還是跟著盤中每輪重抓（15 分鐘節流是後來才加的），一輪就是 2.1 MB ＋ 300 KB ≈ 2.4 MB。
`4.2 GB ÷ 2.4 MB ≈ 1,750 輪`；兩分鐘一輪、工作 14 小時＝一個分頁 420 輪，
反推**約 4～5 個分頁整天開著**。對照 08-25 的 commit（族群樹、K 線連結、行動版底色三條線並行，
18:05～22:25 連續 9 個 commit），多台裝置＋多個 local preview 頁同時指向正式 Supabase，完全合理。

**同樣的行為今天的成本是 2.1 MB → brotli 後約 25 KB，降 84 倍**，4.2 GB 會變成約 50 MB。
所以這個風險沒有被複製到 CDN 架構上，可以結案。

### 還要補的護欄（不是省流量，是省「看不見的狀態」）
...
層級。**下面 🔵 2026-09-12（續）已經找到並修正「手機上傳仍走 Tesseract」的真正原因，不是
admin 權限問題，這裡原本的猜測是錯的。**

### 🔵 2026-09-12（續）真正根因：Worker 併行槽會陣亡、前端心跳誤判提早取消排隊工作

同一天稍晚，使用者用同一支手機、同一個登入身分一次上傳 6 張截圖，回報「一下走 AI
一下走 Tesseract」「感覺只有排隊沒有辨識」。直接查 Supabase：`ocr_workers` 心跳新鮮、
Codex 完全正常，`ocr_jobs` 這次**確實有**新工作列（推翻上一節「卡在 readiness/submit
之前、疑似 admin 權限」的猜測）——5 張 `succeeded`（真的跑了 AI，91～254 秒），
2 張 `status='cancelled'`、`attempt_count=0`，從頭到尾沒被任何 Worker 碰過，是在
排隊時被取消的。
...
1. **`OcrWorkerRunner` 的 3 個並行槽包在同一個 `Task.WhenAll` 裡**，`claim` 落空就
   ...
   要等目前這一整輪（可能卡在一張 254 秒的慢圖）跑完才會被處理。改成 3 個常駐槽，
   Worker 存活期間持續跑，落空時等喚醒信號而不是死掉，槽與槽之間完全獨立不互相
   等待。
2. **前端 `assetAiQueuedWorkerUnavailable()` 用「心跳新鮮度」猜測 Worker 是否離線**，
   排隊超過 30 秒就每輪重查一次、猜測離線就直接取消工作；心跳固定 60 秒一次，
   年齡本來就在 0～60 秒間擺盪，有一半時間會誤判。已完全移除這個心跳重查，已送出
   的工作只信任事實（`queued`/`leased` 本身就是事實）與原本就有的
   `ASSET_AI_OCR_TIMEOUT_MS`（9 分鐘）絕對時限；心跳新鮮度判斷只保留在上傳前
   的一次性 readiness 檢查（未改動，120 秒門檻）。

順手把 `queued` 狀態的輪詢間隔從 700ms/1,500ms 拉長到 3 秒，減少 Supabase 呼叫量；
`.NET 10.0.302` Release 458/458 全綠（含更新的兩個原始碼接線測試），Node 靜態頁
回歸測試 78/78 全綠。**尚未做的**：真正 6 張截圖的端到端實機驗證——沒有登入帳密，
...
<a id="todo-16"></a>
## 🟡 16. 市場切換（台股／美股／日股／韓股／加密貨幣；日韓最高權限入口）

[↑ 回到 TODO 列表](#快速跳轉)

**狀態：`MARKET_SWITCH_PROTOTYPE` 這個 localhost 限定開關已拿掉，市場切換頁籤
現在是正式網站永久功能，所有使用者都看得到。台股維持真實內容、完全沒有重畫
（`initMarketSwitch()` 只在切到非台股市場時用 CSS 隱藏 `.ranking-page`，
`start()` 主流程一行都沒動）。美股／加密貨幣已接上真實 Yahoo Finance 資料，
`MARKET_SWITCH_MOCK` 已刪除。2026-09-12 已大致定案新版熱絡指標的產品語意、輸出數量與第一版權重，
且四市場熱絡指標已完成程式實作。**

**2026-09-23 日韓盤中顯示／更新修復：** 快照完整性仍拒絕未來日期、latest／snapshot 不一致及缺核心資料；但年齡超過 20 分鐘只標示延遲，不再刪除最後一份有效快照。手動切到盤中可在收盤後檢視最後成功快照及其真實交易日／時間；日韓盤中隱藏日期選擇器並清掉舊日期。總覽維持每 5 分鐘，Yahoo 成交排行改為每 20 分鐘最多嘗試一次，失敗也節流。本機 .NET 549/549、Node 146/146 通過；commit `e0bf561b` push 到 `main`，publish-only run `35823304303` 成功，公開 manifest `1790142064`、公開 `site.js` 修正字串均已驗證。Storage 實際狀態為 KR `tradeDate=2026-09-23`／capturedAt 14:05／15 列；JP `tradeDate=2026-09-14`／capturedAt 9/14 14:25／15 列。追查 Actions 歷史確認 9/15 起舊流程曾因缺少 KIS 金鑰退出 134；9/19 改用 Yahoo Screener，9/22 修正已存在 Storage bucket 回 HTTP 400 的 Duplicate 判定。日本 9/21～23 休市，因此這些修正後尚無 JP 開市日可實測；待 9/24 驗證 JP 指標確實更新。前端已顯示舊資料真實日期／時間及非即時警示，不把網站程式發布誤當行情快照更新。

**筆記 #68（2026-09-14，程式、網站發布與盤中首輪驗收完成）：** 使用者要求不分兩階段，而是在同一次實作完成日股／韓股的盤後日線與盤中資料路徑。`backfill-overview --markets jp,kr` 現在只回補兩個市場並由 `asia-market-overview-daily.yml` 在台北 17:30 後寫入 `data` 分支；5 分鐘收集器只在東京／首爾交易時段執行，日股午休不抓取，核心三指數與至少 9 檔產業代表到齊才發佈。盤中快照放到獨立公開 Supabase Storage bucket，版本化快照可長快取、`latest.json` 短快取且只保留最近 30 份；不新增 migration、不讀寫 Postgres，因此不會重演台股全市場盤中直連的資料庫流量。

**2026-09-14 JSON 契約修正：** 公開快照實查已有 `japan`／`korea` 資料，但前端頁籤使用 `jp`／`kr`，因此日韓頁顯示空白。export 現在固定輸出 `jp`／`kr`，前端對舊欄位保留一次相容讀取；核心資料仍必須由品質門檻驗證，不以 fallback 掩蓋缺資料。commit `9bd988bb` 已包含在 publish-only run `34801030426`，公開 manifest `1789354845` 與 `jp`／`kr`（各 3 指數／11 產業）已驗證，並已指向盤中 Storage CDN。

前端只在日韓當地開市且快照交易日／時間仍新鮮時把 5 分鐘暫估接在日線交易日選擇器尾端；任何逾時、讀取失敗、非交易時段或缺少必要資料都保留靜態日線並顯示 warning，不以模板或昨天的產業價格偽裝成即時值。日股 TOPIX-17 對照修正為銀行 `1631.T`、零售 `1630.T`、運輸 `1628.T`。Yahoo Finance 沒有 SLA，且既有直測曾遇 HTTP 429；最新 manifest 已指向盤中 CDN，`jp/latest.json`／`kr/latest.json` 與其 2026-09-14 快照檔均 HTTP 200，風險序列缺少 5 分鐘來源時仍保留 warning 並計入 20% 權重。後續必須持續驗收每輪三指數／至少九產業代表、Storage cache header 與前端回退行為；失敗只保留舊快照並告警，不發布部分盤中資料。

**筆記 #67（2026-09-12，進行中）：** 日股／韓股市場入口已接到正式網站程式，但仍只對最高權限
顯示；一般檢視權限不會看到兩個頁籤。最高權限市場順序固定為台股／美股／日股／韓股／加密貨幣。
日股／韓股已改讀各自的市場總覽 export，不再使用模板示意資料；指數卡顯示個別 0～10 熱絡分數，
11 格產業區塊可切換熱力圖／列表，並沿用前後交易日按鈕。日本先採 TOPIX-17 ETF 代理產業，韓國先採
11 檔產業代表股；兩者只作確認，不是市值權重。若權限降低時正停在日股／韓股，會回到台股；最高權限的五個市場頁籤在手機窄寬度
可水平捲動。版面確認用本機預覽：`http://127.0.0.1:5220/?access=admin`。
...
- 市場導覽已採用正式 U1 版型：市場切換與主頁籤、子頁籤分層排列，主頁籤與子頁籤同為 16px，
  桌面工具列固定在右上角，手機版完整工具／權限區塊放在市場頁籤之前；台股／美股／加密貨幣共用內容外框與手機響應規則，導覽與標題卡之間不繪製整條
...
- **抓取層**（`src/Invest.Web/Infrastructure/MarketData/Overview/`）：
  `MarketOverviewCatalog`（25 檔 symbol 固定名冊：美股四大指數＋VIX＋11 大類股
...
- `MarketOverviewCalculator.DetermineAsOfDate`：這批 symbol「全部到齊」的最新交易日
  （取每個 symbol 自己最新日期的**最小值**）；`StaticSiteExporter` 用它把各市場的 `history`
  各自截到自己的到齊日期才計算，確保同一個面板裡每個數字都對得起
  同一天，超前的 symbol 名單寫進既有 `warnings` 陣列。
- 收盤市場各自新增 `asOf`（目前顯示的交易日）與 `dates`（近 120 天內、整批到齊的可選交易日
  清單）欄位；`StaticSiteExporter.WriteMarketOverviewHistoryAsync` 把每個到齊日各自寫成
  `data/market-overview-{market}-{date}.json`。加密貨幣是 24/7 市場，不提供選擇器。
- **前端**：`mspBuildDateStepper()` 在美股／日股／韓股總覽面板加上前後交易日按鈕（視覺比照台股盤後
...
### 2026-09-12 熱絡指標第一版規格（已實作，尚未發布）

- 0～10 統一解讀為「多頭行情熱絡度／風險偏好」；不再沿用 2026-09-09 草案的
  「只量絕對活動、與方向無關」定義。恐慌性爆量下跌仍應壓低分數。
- 美股輸出道瓊、Nasdaq Composite、費半、S&P 500 與四大指數綜合，共 5 個分數。
  每個個別指數為技術分數 80%＋VIX 20%；綜合分數先按 S&P 500 40%、Nasdaq 25%、
  費半 20%、道瓊 15% 合併，再用指數區塊 80%＋11 大類股量價 20% 組成。
- 加密貨幣輸出 BTC、ETH、SOL 與四幣綜合，共 4 個分數；綜合固定權重為
  BTC 45%、ETH 30%、SOL 15%、DOGE 10%。
- 公式共用 MA20／MA60、MACD、RSI，加密幣與類股 ETF 再用 OBV；最終分數用 EMA3 平滑。
...
- 2026-09-22 追查日韓盤中總覽停在 9/14 的根因：`MarketOverviewIntradaySnapshotPublisher` 只接受外層 HTTP 409，
  未接受 Supabase 外層 HTTP 400、body 為 `BucketAlreadyExists` 的既有 bucket 回應；已補上精確 Duplicate 判定與三項回歸測試，
  commit `acdc81e3` 推送後完整 .NET 測試 532/532。
- `asia-market-overview-intraday.yml` run [`35683895260`](https://github.com/qwe953751/Investment/actions/runs/35683895260)
