...
5. 若 MA 包含當日收盤，完整 MA5／20／60／240 分別需要 **4／19／59／239 筆前置有效收盤**。若三個月圖從第一根顯示 K 棒起就要畫出 MA240，必須在顯示區間前再取 239 筆有效交易日收盤；目前 `backfill-bars 90` 不足以保證這件事。
...
目前選擇器以 `endDate.AddMonths(-3)` 取得三個月顯示區間。如果要讓三個月區間的第一根 K 棒就同時顯示完整 MA5／20／60／240，資料需求是：
...
- [ ] MA 窗口以有效交易日收盤計數；三個月圖若要完整 MA240，從顯示起點向前取 239 筆有效收盤，並取得覆蓋整段資料的公司行動事件。
