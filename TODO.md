...
| 15 | [D+ AI OCR：名稱反查、效能、進度、常駐與實機驗收](#todo-15) | 🔵 2026-09-12 已修三個獨立問題：Worker 3 個並行槽 claim 落空會永久死掉（改常駐槽）、前端心跳誤判提早取消排隊工作（Worker 端已上線，**前端半套是否已隨同一天的 OCR 強制取消功能一起發布需重新查證**）、readiness 探測失敗 fail-closed 單次抖動整批靜默降級（改 5 秒內重試 5 次＋不可用時 10 秒加速重探＋常駐排程 log 檔）；459 個 .NET 測試全綠，**尚待使用者下次整批上傳驗證，且第三個根因是反推、非第一手證據**。另已完成 Windows→Mac→Tesseract 固定跨機接力並重啟公司 Windows（筆記 #61 收尾）；**家裡 Mac 仍待使用者重載 LaunchAgent，跨機接力未實機驗證**。另仍待 Claude Pro 登入、Golden Set、Windows 每張 ≤30 秒與長期斷線復原驗收 |
...
- 日股與韓股已比照美股接入同一引擎；`^JNIV`、`^VKOSPI` 與韓國產業代表名冊仍需實際回補驗證，可由設定替換。
