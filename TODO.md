...
| 15 | [D+ AI OCR：名稱反查、效能、進度、常駐與實機驗收](#todo-15) | 🟡 2026-09-13 查明第五個獨立問題並修好：連測多次幾乎都直接走 Tesseract，根因是 `readiness` 的時間門檻 `Number(null)=0` 被 clamp 成 15 秒、跟 Worker 心跳週期（67 秒）相位不同步，78% 機率誤判離線。已重構為樂觀語意單一入口＋事實優先存活判定＋工作層級 stall 偵測（`db/054`），並完成 Worker 端連線旗標／心跳降頻／CLI 探測快取；493 個 .NET＋94 個 Node 測試全綠。**🟡 DB migration 與 Edge Function 已於 2026-09-13 部署到正式 Supabase；Worker EXE 重新 build 這一步仍待使用者在公司 Windows 與家裡 Mac 各自執行（需 Stop/Start 排程工作，會短暫中斷服務）**，詳見 [版本紀錄.md](Doc/版本紀錄.md) 最新一節。09-13 稍早已修第四個獨立問題：「強制取消辨識」與重整恢復流程搶同一個全域狀態，取消後彈回掃描中、下一批可能被誤 abort（改用世代編號＋取消時同步清空本機清單）；並補上「取消後 Worker 也真的停手」，順手修掉讓這個檢查生效的前提 bug（`ai_recognition` 階段名稱不合法，`db/052`＋`ocr-jobs` v15）。461 個 .NET 測試全綠，正式資料庫 rollback 測試四項斷言全過；**前端修正沒有真正瀏覽器端到端測試**。09-12 已修三個獨立問題並重啟公司 Windows：並行槽陣亡、前端心跳誤判、readiness fail-closed（**前端那半是否已隨強制取消功能一起發布需重新查證**）。另已完成 Windows→Mac→Tesseract 固定跨機接力（筆記 #61 收尾）；**家裡 Mac 仍待使用者重載 LaunchAgent，跨機接力未實機驗證**。另仍待 Claude Pro 登入、Golden Set、Windows 每張 ≤30 秒與長期斷線復原驗收 |
| 16 | [市場切換（台股／美股／日股／韓股／加密貨幣；日韓最高權限入口）](#todo-16) | 🟡 日韓日線與 `jp`／`kr` JSON 契約已修正；本輪網站發布驗證完成後，等下一個交易日首輪盤中 Storage 快照，再跑一次 export／發布驗收 CDN manifest |
...
**筆記 #68（2026-09-13，程式完成、待真實資料驗收）：** 使用者要求不分兩階段，而是在同一次實作完成日股／韓股的盤後日線與盤中資料路徑。`backfill-overview --markets jp,kr` 現在只回補兩個市場並由 `asia-market-overview-daily.yml` 在台北 17:30 後寫入 `data` 分支；5 分鐘收集器只在東京／首爾交易時段執行，日股午休不抓取，核心三指數與至少 9 檔產業代表到齊才發佈。盤中快照放到獨立公開 Supabase Storage bucket，版本化快照可長快取、`latest.json` 短快取且只保留最近 30 份；不新增 migration、不讀寫 Postgres，因此不會重演台股全市場盤中直連的資料庫流量。

**2026-09-13 JSON 契約修正：** 公開快照實查已有 `japan`／`korea` 資料，但前端頁籤使用 `jp`／`kr`，因此日韓頁顯示空白。export 現在固定輸出 `jp`／`kr`，前端對舊欄位保留一次相容讀取；核心資料仍必須由品質門檻驗證，不以 fallback 掩蓋缺資料。網站重新 export／發布與公開 manifest 驗證列在 `Doc/版本紀錄.md` 最新一節。

前端只在日韓當地開市且快照交易日／時間仍新鮮時把 5 分鐘暫估接在日線交易日選擇器尾端；任何逾時、讀取失敗、非交易時段或缺少必要資料都保留靜態日線並顯示 warning，不以模板或昨天的產業價格偽裝成即時值。日股 TOPIX-17 對照修正為銀行 `1631.T`、零售 `1630.T`、運輸 `1628.T`。Yahoo Finance 沒有 SLA，且既有直測曾遇 HTTP 429；本次為週日，沒有發出實際 Yahoo 請求、沒有建立 bucket、沒有寫入行情或發布網站。下一個交易日必須驗收兩年日線、每輪三指數／至少九產業代表、`^JNIV`／`^VKOSPI` 可用性、Storage cache header 與前端回退行為；失敗只保留舊快照並告警，不發布部分盤中資料。
