...
白名單」方向**沒有採用**，改成更簡單的密碼登入＋固定視角：訪客（不登入）／
持倉檢視者／監控者／最高權限。原因是 Google OAuth 對靜態站（無後端）串接複雜、且使用者
只有自己和另一個信任對象要登入，不需要白名單擴充彈性。

實作方式：

- 頁首新增獨立登入列（`#access-bar`），隨時顯示目前權限，密碼輸入比對四組
  ...
  `holdings@investment.local` → 持倉檢視者、固定只讀 Frank 所有帳號持股；
...
- 前端已完整實作並端到端驗證（真密碼登入、頁籤依權限篩選、自動恢復都測過），
  原有帳號 commit `6ddd72d4`（`main` 已推送）；本次持倉檢視者的程式與模板已完成，
  等本輪 commit／發布後補上正式驗證資訊。`db/029_permission_accounts.sql` 已用
  Management API 套用到正式 Supabase；2026-09-07 新增的財神最高權限帳號也已確認可正常登入。

### 尚未討論／待驗收

- **RLS 尚未收權限**：`notes`／`topic_edits`／資產相關表（含持倉、出入金與每日快照）目前仍是 anon
  可讀可寫，跟登入前完全一樣——本次持倉檢視者也只做前端登入、頁籤切換與唯讀呈現，故意不動 RLS。
...
- **市場切換（第一層）**：分段控制（單一膠囊容器，選中項目浮起白底），
  跟底下第二層的頁籤／內容視覺分層，不是兩排疊放的等重選項。
- **指數**：小方塊樣式（一行放三檔指數，比原本大卡片版緊湊）。
- **整體版面**：緊湊列表（拿掉卡片邊框改用分隔線，頁面比較短）。
- **板塊／賽道表現**：改成真熱力圖（方塊大小依市值權重分大中小三級，
  跟漲跌幅無關；顏色深淺依漲跌幅強度），標題列右上角有 ▦／☰ 圖示可切換
  熱力圖／列表兩種檢視。
- 其餘曾經比較過的替代方案（底線頁籤、卡片式市場切換、表格列／橫向卡指數、
  分區卡片／雙欄版面）已從程式碼刪除，`site.js` 只保留這一版，沒有殘留的
  選擇器 UI。

### 已上線（正式網站）

- 拿掉 `MARKET_SWITCH_PROTOTYPE`，市場切換頁籤對所有使用者永久顯示，
  不再限定 localhost 或 `?preview=` 網址。
- 進站預設台股：`.ranking-page` 原樣顯示，真實渲染／初始化流程完全不受影響。
- 切到美股／加密貨幣：`initMarketSwitch()` 幫 `<body>` 加 class，用既有
...
- 市場導覽已採用正式 U1 版型：市場切換與主頁籤、子頁籤分層排列，主頁籤與子頁籤同為 16px，
  工具列固定在右上角；台股／美股／加密貨幣共用內容外框與手機響應規則，導覽與標題卡之間不繪製整條
  交界線；資產／筆記不因切換市場而鎖定。本輪程式與文件已推送並發布，詳見版本紀錄。
