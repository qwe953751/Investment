...
public sealed class StaticKLineAssetTests
{
    ...
    public void 資產持倉表頭全部六個欄位皆可排序且缺值沉底()
    {
        ...
        Assert.Contains(
            ...
            script,
            StringComparison.Ordinal);
        Assert.DoesNotContain("button.title = key === 'priceChange'", script, StringComparison.Ordinal);
        Assert.Contains("assetCurrency(Math.abs(amount), currency)", script, StringComparison.Ordinal);
        Assert.Contains("`${amountText}（${Math.abs(percent)}%）`", script, StringComparison.Ordinal);
    }
    ...
    public void 熱絡卡片提供就地走勢分析並讀取近三個月交易日()
    {
        ...
        Assert.Contains("marketHeatRangePercentLine(points, point => point.volumeRatio)", script, StringComparison.Ordinal);
        ...
        Assert.Contains("function marketHeatIndexKLineBars", script, StringComparison.Ordinal);
        Assert.Contains("indexKLinePriceRange(bars)", script, StringComparison.Ordinal);
        Assert.Contains("marketHeatRangePercentLine", script, StringComparison.Ordinal);
        Assert.DoesNotContain("marketHeatPercentLine(points, point => point.twseIndex)", script, StringComparison.Ordinal);
        Assert.DoesNotContain("marketHeatPercentLine(points, point => point.tpexIndex)", script, StringComparison.Ordinal);
        Assert.Contains("marketHeatRangePercentLine(points, point => point.twseTurnover)", script, StringComparison.Ordinal);
        Assert.Contains("marketHeatRangePercentLine(points, point => point.tpexTurnover)", script, StringComparison.Ordinal);
        Assert.DoesNotContain("marketHeatPercentLine(points, point => point.volumeRatio)", script, StringComparison.Ordinal);
        Assert.DoesNotContain("marketHeatPercentLine(points, point => point.twseTurnover)", script, StringComparison.Ordinal);
        Assert.DoesNotContain("marketHeatPercentLine(points, point => point.tpexTurnover)", script, StringComparison.Ordinal);
        Assert.Contains("domain: [0, 100]", script, StringComparison.Ordinal);
        Assert.Contains("指數依三個月 K 線縱軸高低點換算", script, StringComparison.Ordinal);
        Assert.Contains("量能與成交額各自依三個月最低／最高值換算", script, StringComparison.Ordinal);
        Assert.Contains("void loadIndexKLineData().then(() =>", script, StringComparison.Ordinal);
